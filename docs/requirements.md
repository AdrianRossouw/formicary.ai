...
| REQ-037 | Every page SHALL declare a favicon via `_layouts/default.html`. The `<head>` SHALL include: an SVG icon (`favicon.svg`), PNG fallbacks at 48×48, 32×32, and 16×16 (`favicon-48.png`, `favicon-32.png`, `favicon-16.png`), and an Apple touch icon (`apple-touch-icon.png`). All favicon files SHALL be present at the repository root so Jekyll copies them verbatim to `_site/`. |
