...
`tests/scout.test.js` and `tests/traceability.test.js` via Vitest (`npm test`).
Build: `bundle exec jekyll build`.

`assets/css/tokens.css` is the design system source of truth — all colour,
spacing, and type values live here. Do not hardcode design values elsewhere;
make changes here first. Class names must match the brief in
`docs/_brief/design_brief/` exactly; read those HTML files before implementing
